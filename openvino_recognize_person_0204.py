...
b_r, c_r, h_r, w_r = rec_input.shape  # Corrected to use rec_input.shape

# 載入參考圖像並提取特徵
ref_image = cv2.imread('kai.jpg')
if ref_image is None:
    raise FileNotFoundError("無法載入 image.jpg，請確認檔案路徑")
...
ref_face = None
for detection in ref_results[0][0]:  # Assuming single batch
    conf = float(detection[2])
    if conf > 0.5:  # Higher threshold for reference image
...
while True:
    ...
    # 繪製預測框
    for detection in results[0][0]:  # Adjusted for OpenVINO output format
        ...
        if conf > 0.3:
            ...
            label = "Kai" if similarity > 0.6 else "Unknown"  # 閾值可調整
            cv2.putText(
                frame, f"{label} ({similarity:.2f})", (x_min, y_min - 10),
